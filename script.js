...
const searchTemp = `<form class="search__form">
<div class="search">
    <input name="search" class="search__input" type="search" placeholder="Search for item...">
    <button name="search" class="search__magnifier" type="submit">
        <img src="img/magnifier.png" alt="magnifier">
    </button>
</div>
</form>`;
const cartTemp = `<div class="cart">
<div class="cart__amount"></div>
<details class="cart__details">
    <summary class="cart__summary"><img src="img/cart.png" alt="cart">
    </summary>
    <div class="cart__box">
        <div class="cart__items">
        </div>
        <div class="last_in_col">
            <div class="cart__total">
                <div class="cp">TOTAL</div>
                <div class="cart__total_price cp">$0.00</div>
            </div>
...
let searchComponent = Vue.component('search-component', {
    template: searchTemp
});
...
class CatalogList {
    ...
    render() {
        ...
        let addToCartArr = document.querySelectorAll('.addToCart');
        for (let but of addToCartArr) { but.addEventListener('click', e => this.addToCart(e)) };
    }
    addToCart(e) {
        if (e.target.className === 'addToCart' || (e.target.closest('button') && e.target.closest('button').className === 'addToCart')) { //Ищу класс кнопки
            ...
            let cartPrId = cartList.product.findIndex(i => i.id == prId); //ищу индекс в массиве корзины
            if (cartPrId == -1) { //проверяю, нет ли его в корзине
                async function getPr() {
                    let newProd = await $productArray.then(r => { return r.find(i => i.id == prId); });
                    console.log(newProd);
                    cartList.product.push(new ProductInCart(newProd)); //добавляю в массив корзины новый объект ПродуктКорзины, 
                ...
                cartList.product[cartPrId].quantity++; //Если он уже есть в корзине - увеличиваю его количество
                cartList.render();
            }
            cartList.renderTotal();
            cartList.renderAmount();
            return;
        }
    };
}
